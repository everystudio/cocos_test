...
void GameScene::initForVariables()
{
	...
	// コマの一辺の長さを取得
	BlockSprite* pBlock = BlockSprite::createWithBlockType(EBlockTypeRed);
	m_fBlockSize = pBlock->getContentSize().height;
...
CCPoint GameScene::getPosition(int _iPosIndexX, int _iPosIndexY)
{
...
// タグ取得
int GameScene::getTag(int _iPosIndexX, int _iPosIndexY)
{
	return eTagBaseBlock + _iPosIndexX * 100 + _iPosIndexY;
}

// 未使用のブロックタグを取得する
int GameScene::getEmptyBlockIndex()
{
	int iRet=-1;
...
cocos2d::CCPoint GameScene::getBlockPos(int _iX , int _iY)
{
	float fOffsetX = m_pBackground->getContentSize().width * 0.168;
	float fOffsetY = m_pBackground->getContentSize().height * 0.028;
...
bool GameScene::addBlockBase(BlockBase *_pBlockBase,int _iX , int _iY)
{
...
void GameScene::showBlock()
{
	for(int x = 0 ; x < MAX_BLOCK_X ; x++){
		for(int y = 0 ; y < MAX_BLOCK_Y-2 ; y++ ){
			...
			// まじめな方
			BlockBase* pBlockBase = BlockBase::createWithBlockType(eBlockType);
...
// タッチ開始イベント
bool GameScene::ccTouchBegan(cocos2d::CCTouch *_pTouch, cocos2d::CCEvent *_pEvent)
{
...
int GameScene::getBlockIndexX( cocos2d::CCPoint _tPoint)
{
	int iIndexX = 0;
	float fOffsetX = m_pBackground->getContentSize().width * 0.168;
	fOffsetX = BLOCK_OFFSET_X;
...
cocos2d::CCPoint GameScene::getHandPosition( cocos2d::CCPoint _tTouchPoint )
{
...
void GameScene::moveHandPosition( HandBase* _pHandBase , cocos2d::CCPoint _tPoint )
{
...
void GameScene::setHandPosition(HandBase *_pHandBase, cocos2d::CCPoint _tPoint)
{
...
void GameScene::blockHit( CCNode* _pNode )
{
...
void GameScene::moveBlock( int _iBlockTag , CCPoint _tPoint)
{
...
void GameScene::shotBlock( int _iBlockTag , cocos2d::CCPoint _tPoint , int _iSearchIndex)
{
	...
	CCCallFuncN* func = CCCallFuncN::create(this,callfuncN_selector(GameScene::blockHit));
...
// タッチ終了イベント
void GameScene::ccTouchEnded(cocos2d::CCTouch *_pTouch, cocos2d::CCEvent *_pEvent)
{
	// タップポイント
...
// タップされたコマのタグを取得
void GameScene::getTouchBlockTag(cocos2d::CCPoint _tTouchPoint, int &_tag, EBlockType &_EBlockType)
{
	for(int x = 0 ; x < MAX_BLOCK_X ; x++ ){
		for( int y = 0 ; y < MAX_BLOCK_Y ; y++ ){
			...
			if( pNode && pNode->boundingBox().containsPoint(_tTouchPoint)){
				_tag = iCurrentTag;
				_EBlockType = ((BlockSprite*)pNode)->getBlockType();
				return;
			}
		}
	}
...
// タップされたコマのタグを取得
void GameScene::getTouchBlockIndex(cocos2d::CCPoint _tTouchPoint, int &_iBaseIndex, EBlockType &_EBlockType)
{
	for(int x = 0 ; x < MAX_BLOCK_X ; x++ ){
		for( int y = 0 ; y < MAX_BLOCK_Y ; y++ ){
			int iIndex = x+y*MAX_BLOCK_X;
...
// コマ配列にあるか検索
bool GameScene::hasSameColorBlock(std::list<int> _listBlockTag , int _iSearchBlockTag)
{
...
bool GameScene::hasSameIndexBlock(std::list<int> _listBlockIndex , int _iSearchBlockIndex)
{
	list<int>::iterator it;
...
std::list<int> GameScene::getSameColorBlockTags(int _iBaseTag, EBlockType _EBlockType)
{
...
// タップされたこまと同じ色でかつ接しているコマの配列を返す
std::list<int> GameScene::getSameColorBlockIndexs(int _iBaseIndex)
{
...
// 配列のコマを削除
void GameScene::removeBlock2(std::list<int> _listBlockIndexs)
{
	...
	while(it != _listBlockIndexs.end()){
		...
		if(pBlock){
			...
			// コマを削除するアクションを生成
			CCCallFuncN* pFunc = CCCallFuncN::create(this,callfuncN_selector(GameScene::removingBlock));
...
// 配列のコマを削除
void GameScene::removeBlock(std::list<int> _listBlockTags, EBlockType _EBlockType)
{
	bool bFirst = true;
...
// コマの削除
void GameScene::removingBlock(cocos2d::CCNode *_pBlock)
{
	_pBlock->removeFromParentAndCleanup(true);
	return;
}

GameScene::TPositionIndex GameScene::getPositionIndex(int _iTag)
{
...
void GameScene::setNewPosition(int _iTag, GameScene::TPositionIndex _tPosIndex)
{
	BlockSprite* pBlockSprite = (BlockSprite*)m_pBackground->getChildByTag(_iTag);
	int iNextPosY = pBlockSprite->getNextPosY();
	if(iNextPosY == -1 ){
		iNextPosY = _tPosIndex.iY;
	}

	// 移動先の位置をセット
	pBlockSprite->setNextPos( _tPosIndex.iX , --iNextPosY);
	return;
}

// 消えたコマを埋めるように新しい位置をセット
void GameScene::searchNewPosition1(std::list<int> _listBlocks)
{
	// 消えるコマ数分のループ
...
void GameScene::moveBlock()
{
	...
	while(it1 != m_vecBlockTypes.end()){
		...
		while(it2 != m_mapBlockTags[*it1].end()){
			BlockSprite* pBlockSprite = (BlockSprite*)m_pBackground->getChildByTag(*it2);
			int iNextPosX = pBlockSprite->getNextPosX();
			int iNextPosY = pBlockSprite->getNextPosY();

			if( iNextPosX != -1 || iNextPosY != -1){
				// 新しいタグをセットする
				int iNewTag = getTag(iNextPosX,iNextPosY);
				pBlockSprite->initNextPos();
				pBlockSprite->setTag(iNewTag);
...
// コマ削除後のアニメーション
void GameScene::movingBlocksAnimation1(std::list<int> _listBlocks)
{
	// コマの新しい位置をセットする
